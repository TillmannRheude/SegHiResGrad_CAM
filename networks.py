...
class Vanilla_UNet_2d(nn.Module):
    ...
    def __init__(self, 
        ...
        # Encoder
        self.encoder = nn.ModuleList([Conv_Block_2d(encoder_channels[i], encoder_channels[i+1]) for i in range(len(encoder_channels)-1)])

        # single upconvolution
        self.upconv = nn.ConvTranspose2d(in_channels=decoder_channels[0], out_channels=decoder_channels[1], kernel_size=2, stride=2)
    
        # Decoder
        decoder_depth = len(decoder_channels)-3
        self.decoder = nn.ModuleList([Upconv_Block_2d(in_channels=decoder_channels[i], mid_channels=decoder_channels[i+1], out_channels=decoder_channels[i+2]) for i in range(decoder_depth)])

        # Final layer
        self.final_layer = nn.ModuleList([
                        Single_Conv_Block_2d(in_channels=decoder_channels[-3], out_channels=decoder_channels[-2]),
                        Single_Conv_Block_2d(in_channels=decoder_channels[-2], out_channels=decoder_channels[-2]),
                        Single_Conv_Block_2d(in_channels=decoder_channels[-2], out_channels=decoder_channels[-1], with_relu_batchnorm=False, conv_kernel_size=1, conv_padding_size=0)
                        ])
    ...
    def pad_tensor(self, tensor_with_final_shapes, x):
        ...
        if(h_e > h_x):
            ...
            if(w_e > w_x):
                padded_tensor_x = F.pad(input=padded_tensor_x, pad=(1, 0, 0, 0), mode="constant", value=0)

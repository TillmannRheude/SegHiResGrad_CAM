...
class Dataset_2d(Dataset):
    ...
    def __init__(
        ...
        self.paths_images = sorted(
            glob.glob(f"{dir_root}images/*.{self.img_extension}")
        )
        ...
        if "resize" in self.transform:
            ...
            if self.resize is not None:
                self.array_transformations.append(
                    T.Resize(
                        size=self.resize, interpolation=T.InterpolationMode.NEAREST
                    )
                )  # (250,358)
    ...
    def __getitem__(self, idx):
        ...
        # Load rgb jpg image
            ...
            image = cv2.imread(
                self.paths_images[idx]
            )  # the workaround with cv2 for solving a warn-message

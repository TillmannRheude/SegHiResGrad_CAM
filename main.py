...
from utils import set_encoder_decoder, create_model, create_save_folders, Model_Inference
...
parser.add_argument("--amount_classes", type=int, default=34, # 33 for opg, 17 for word, 2 for binary, 34 for cityscapes
...
# Model choice
parser.add_argument("--model", type=str, default="vanilla_unet", 
                    help="possible values: vanilla_unet")
...
parser.add_argument("--cam", type=str, default="hirescam",
                    help="Decide whether to use Seg-Grad CAM (gradcam) or Seg-HiRes-Grad CAM (hirescam).")
parser.add_argument("--px_set", type=str, default="point",
                    help="Decide whether to use image, class, point or zero. Best results with class.")
parser.add_argument("--px_set_point", type=str, default=(300, 300),
...
encoder_channels, decoder_channels = set_encoder_decoder(color=args.color, amount_classes=args.amount_classes, depth=5)

# Create the model 
model, path_save_folder_grad_cam, path_load_model = create_model(modelname=model_name, 
...
if __name__ == '__main__':
    ...
    dataloader = build_dataloader(dir_root=args.test_path, transform={**transform_resize}, num_workers=1,
                                batch_size=1, return_filenames=True, shuffle=False,
                                dataset_color=args.color, classes=classes)
    # Load the respective model
    Model_Class = Model_Inference(model=model, 
